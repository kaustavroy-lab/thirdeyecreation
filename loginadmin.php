<?php
   session_start();
   error_reporting(0);
   define('ADMIN_PASS', '$2a$10$NcQZ.zRpsjQkyXwPTS9ATemecaxSa6su0ZenCBxSN/q1FoIstz2qi');
   $session_timeout = 600;
   ...
   if (!empty($action))
   {
      ...
      if ($action == 'update')
      {
         ...
         foreach($items as $line)
         {
            if ($index == $userindex)
            {
               ...
               {
                  $values[1] = password_hash($_POST['password'], PASSWORD_BCRYPT);
               }
      ...
      if ($action == 'create')
      {
         ...
         fwrite($file, '|');
         fwrite($file, password_hash($_POST['password'], PASSWORD_BCRYPT));
         fwrite($file, '|');
